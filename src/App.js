...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  async function handleSubmit(e) {
    ...
    if (text && text.length <= 200 && isValidHttpUrl(source) && category) {
      //3. update fact to Supabase and receive the new object
      setIsUploading(true);
  ...
  return (
    <form className="fact-form" onSubmit={handleSubmit}>
      ...
      />
      <span>{200 - text.length} characters remaining</span>
      <input
        type="text"
...
function CategoryFilter({ setCurrentCategory }) {
  return (
    <aside>
      <ul>
        <li className="category" onClick={() => setCurrentCategory("all")}>
...
function Fact({ fact, setFacts }) {
  ...
  return (
    <li className="fact">
      {disputed ? <span className="disputed">DISPUTED!⛔</span> : null}
      <p>
        {fact.text}
        <a
          className="source"
